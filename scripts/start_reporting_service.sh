...
LOG_FILE="$PROJECT_DIR/automation_report.log"

echo "$(date): Reporting service started. Target time: $TARGET_TIME daily." >> "$LOG_FILE"

while true; do
    CURRENT_TIME=$(date +"%H:%M")
    
    if [ "$CURRENT_TIME" == "$TARGET_TIME" ]; then
        echo "$(date): Triggering daily report..." >> "$LOG_FILE"
