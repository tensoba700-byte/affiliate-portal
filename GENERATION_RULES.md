...
## 7. 記事の公開スケジュールルール（必須・変更禁止）

記事の投稿は「書き溜め予約投稿システム」のみを使用する。この方式以外で記事を公開してはならない。

1. **publishDateの分散設定**：
   - publishDateを朝07:00 JSTと夜19:00 JSTに交互に分散して設定する。
2. **GitHubへのプッシュ**：
   - 生成した記事をGitHubにpushする（Vercelが即デプロイを行う）。
3. **Next.jsによる公開制限**：
   - Next.jsが記事の `publishDate` を見て、指定された時間まで非公開にする。
4. **自動公開**：
   - 指定した時間が来たら、自動で公開される。
